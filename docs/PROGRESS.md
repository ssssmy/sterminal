...
- [x] OpenSSH config 导入：选择 `~/.ssh/config` 文件，预览解析结果后批量导入主机
- [x] STerminal JSON 导出：将主机（含分组）、命令片段、设置导出为加盖时间戳的 JSON 备份文件
- [x] STerminal JSON 导入：从备份文件恢复数据（可选合并或覆盖）
- [x] 清除本地数据：清空本地 SQLite 中的主机/片段/设置等数据
- [x] 清除全部数据：清除本地数据同时通知云端删除同步数据
