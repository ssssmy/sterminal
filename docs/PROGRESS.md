...
> 更新日期: 2026-04-29 | 当前阶段: P0-P3 全部完成，Design-First 已完成；近期补齐 SFTP 拖拽上传、片段导入导出 + Ctrl+Shift+S 快捷面板、Dynamic SOCKS5 转发 + 流量统计、SSH Agent 集成、KnownHosts 管理、Vault 快捷填充
...
| **P1 核心增强** | SFTP文件管理(含拖拽上传)、命令片段(含导入导出+快捷面板)、端口转发(L/R/Dynamic+流量统计)、设置完善、云同步对接 | ✅ 已完成 |
| **P2 进阶功能** | SSH密钥管理(含SSH Agent)、KnownHosts管理、Vault密钥库(含Ctrl+Shift+V填充)、回放器、日志审计(20种事件)、GIF导出 | ✅ 已完成（剩OAuth登录延后至P4） |
| **P3 体验优化** | 自动补全、命令面板、主题引擎、快捷键自定义、数据管理、自动更新、系统托盘、CLI、URI Scheme、关于页 | ✅ 已完成 |
| **Design-First** | 设计Token、动效系统、首次向导、环境感知边框、SSH连接动画、复制反馈 | ✅ 已完成 |
| **测试** | 204个单元+集成测试（Vitest + 内存SQLite） | ✅ 已完成 |
| **CI/CD** | GitHub Actions 三平台构建 + Windows免安装版 + tag自动发布 | ✅ 已完成 |
| **P4 基础设施** | OAuth登录、服务端迁移PostgreSQL/MySQL、订阅付费系统、代码签名与公证 | ⏳ 待开发 |
...
| 7 | ~~前端 CommandPalette 未实现~~ | ~~低~~ | ✅ P3 已完整实现（6 类搜索：命令/主机/片段/终端/端口转发/设置） |
...
- [x] 拖拽上传（从系统文件管理器拖入）— `SftpPanel.vue` `@dragover` / `@drop` + `getPathForFile` 解析路径
...
- [x] 片段导入/导出（`DataSettings.vue` 提供 JSON 备份导入导出）
- [x] 快捷面板 Ctrl+Shift+S（`SnippetQuickPanel.vue` + `ui.store` openSnippetQuickPanel，模糊搜索/键盘导航/直接执行到当前活跃终端）
...
- [x] Dynamic 转发 SOCKS5（`port-forward.handler.ts:399` createDynamicTunnel，本地监听 SOCKS5 端口经 SSH 转发到任意目标）
- [x] 流量统计（每条隧道独立 `bytesIn`/`bytesOut` 计数，通过 status IPC 推送到渲染层）
...
- [x] SSH Agent 集成（`key.handler.ts` getAgentFingerprints / agent_load / agent_unload，`auto_load_agent` 字段控制启动时自动加载，ssh.handler 通过 `SSH_AUTH_SOCK` 走 agent 认证）

#### 5.2.2 已知主机管理 ✅ 已完成
- [x] `KnownHostsSettings.vue` — 已知主机指纹列表（含搜索/删除/批量管理）
- [x] SSH 连接时主机密钥验证对话框（首次连接弹窗确认、指纹变更高危警告）
- [x] 添加/删除已知主机
- [x] 指纹算法展示（SHA256）
...
- [x] 自动填充到终端（Ctrl+Shift+V，`VaultFillDialog.vue` + `ui.store` openVaultFillDialog，模糊搜索条目并直接发送到活跃终端）
...
- [x] GIF 导出（canvas + gif-encoder-2，带 STerminal 水印）

#### 5.2.5 日志与审计 ✅ 已完成
- [x] `LogsAuditView.vue` — 操作日志列表 + 过滤 + 搜索 + 分页
- [x] 记录 20 种关键操作（SSH 连接/断开/错误、SFTP 上传/下载/删除/创建目录、主机 CRUD、密钥管理、Vault 操作、设置变更、数据导入导出）
- [x] 日志导出（CSV/JSON）
- [x] 自动清理策略（保留天数可配置）
...
#### 5.3.1 终端自动补全 ✅ 已完成
- [x] 命令历史 + 片段建议（可扩展 provider 架构）
- [x] 右键智能操作（有选中 → 复制，无选中 → 粘贴）
...
#### 5.3.3 主题自定义 ✅ 已完成
- [x] 主题引擎（WCAG AA 对比度验证、10+ 预设、自定义 CRUD、JSON 格式）
- [x] 设计 Token 体系（颜色/间距/阴影/动效/布局/z-index）
- [x] 自定义强调色（Element Plus 全套变量动态覆盖，深浅色都生效）
- [x] 动效系统（spring curves、prefers-reduced-motion 自动降级）

#### 5.3.4 快捷键自定义 ✅ 已完成
- [x] 快捷键设置页面（KeybindingsSettings.vue）
- [x] keybinding.service.ts（解析/匹配/冲突检测/DB 持久化）
- [x] 恢复默认 + CmdOrCtrl 跨平台支持

#### 5.3.5 数据管理 ✅ 已完成
- [x] `DataSettings.vue` — 数据管理设置页（OpenSSH 导入 / JSON 导出导入 / 清除数据）
- [x] `ssh-config-parser.ts` — 解析 `~/.ssh/config`，将 Host 块转换为 STerminal 主机记录
- [x] CLI 安装/卸载（设置页一键安装 sterminal 命令到系统 PATH）

#### 5.3.6 应用更新 ✅ 已完成
- [x] 自动检查更新（electron-updater，启动 10 秒后检查）
- [x] 更新通知栏（可用 → 下载进度 → 重启安装）
- [x] GitHub Releases 发布（tag 触发 CI 自动构建上传）

#### 5.3.7 系统集成 ✅ 已完成
- [x] 系统托盘（最小化到托盘、快速连接最近 10 台主机、紫色 S 品牌图标）
- [x] URI Scheme（`sterminal://connect?name=xxx` 深度链接，右键主机复制链接）
- [x] CLI 工具（`sterminal ssh/sftp/connect/open/new` 8 个命令）
- [x] 深色/浅色模式跟随系统（ui.store matchMedia 监听）
- [x] 标签页右键菜单（重启/复制/固定/关闭/关闭右侧）+ 拖拽排序
- [x] 首次运行向导（3 步：导入 SSH 配置 → 选主题 → 开始）
- [x] 关于页面（版本号 + 功能概览 + GitHub/反馈链接）
